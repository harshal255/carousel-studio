...
function parseColor(colorStr: string): { rgb: Rgb; a: number } | null {
  if (!colorStr) return null
  const trimmed = colorStr.trim().toLowerCase()

  // 1. Check for rgb / rgba
  if (trimmed.startsWith('rgb')) {
    const match = trimmed.match(/rgba?\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)(?:\s*,\s*([\d.]+))?\)/)
    if (match) {
      return {
        rgb: {
...
export function ColorPicker({
  ...
  className = "w-full max-w-[320px] space-y-3 rounded-xl border bg-background p-3 shadow-sm",
}: ColorPickerProps) {
  const parsed = parseColor(value) ?? { rgb: { r: 124, g: 58, b: 237 }, a: 1 }
  ...
  return (
    <div className={className}>
      <div className="relative h-44 w-full overflow-hidden rounded-lg border" style={checkerboard}>
        <div
      ...
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
        ...
        <Input
          ...
          onChange={(event) => {
            ...
            if (format === 'hex') {
              const parsedHex = parseColor(raw)
              if (!parsedHex) return
      ...
      <div className="flex flex-wrap gap-2">
        {swatches.map((swatch) => (
          <button
            ...
            onClick={() => {
              const parsedSwatch = parseColor(swatch)
